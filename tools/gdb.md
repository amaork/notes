...
在 GDB 调试的时候，宏是不能使用 `print(p)` 命令来打印的，因为宏是预编译的。如果要想在调试的时候调试宏，需要在编译的时候加上 `-ggdb3` 的参数，这样就可以在 gdb 中使用 `print(p)` 命令和 `info macro` 以及 `macro expand` 命令展开宏了。
...
- enable 和 disable 使用暂时的启用和禁用某个断点，需要指定断点的编号。设置完成之后可以通过 info b 命令来查看。

- 设置条件断点：条件断点是只有在添加的条件满足的情况下，才会产生断点，这个在调试循环的时候非常有用，其语法如下：

		break [where] if [condition]

		where 是要断点的行数

		condition 是条件，条件为真则产生断点

		例：

		break 10 if idx == 100

		当 idx 等于 100 的时候，在第 10 行产生断点
...
- `info thread` 查看当前的程序的线程

- `thread <id>` 是用来切换到 id 指定的线程上

- `break file.c:100 thread all` 设置所有的线程运行到 `file.c` 的第 100 行都会设置断点，可是通过这条命令设置线程函数的断点

- `set scheduler-locking off | on | step` `off` 参数调试时不锁定任何线程，所有的线程都执行，这个默认值。`on` 只有调试的线程会按照调试指令执行。`step` 单步调试的时候，除了 `next` 过一个函数以外只有当前线程会执行。
